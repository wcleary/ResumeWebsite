...
(function ($, window, document, undefined) {
	...
	var Instagram = {
        ...
        getRecentMedia: function () {
			var self = this,
				getData = '/users/' + self.options.userID + '/media/recent/?' + self.accessData.clientID + '&access_token='+ self.accessData.accessToken +'';
				...
				for (var i = max-1; i >= 0; i--) {
					if (setSize == "small") {
						...
						if(results.data[i].caption==null){
							self.$elem.after("<li class='item-thumbs span3 instagram '><a title='No caption!!!!!' target='_blank' href='" + results.data[i].link + "'><img src='" + size + "'></img></a>");
						}else
							self.$elem.after("<li class='item-thumbs span3 instagram '> <a title='" + results.data[i].caption.text + "' target='_blank' href='" + results.data[i].link + "'><img src='" + size + "'></img></a>");
					}
					
						//self.$elem.append("<li class='item-thumbs span3 instagram '> <!-- Fancybox - Gallery Enabled - Title - Full Image --> <a class='hover-wrap fancybox' data-fancybox-group='gallery' title='The Mountains' href='_include/img/work/full/image-03-full.jpg'> <span class='overlay-img'></span><span class='overlay-img-thumb font-icon-plus'></span></a><!-- Thumb Image and Description --><a title='" + results.data[i].caption.text + "' target='_blank' href='" + results.data[i].link + "'><img src='" + size + "'></img></a>");
            			//self.$elem.append("<img src='" + size + "'></img></a>");
